class PlayersController < ApplicationController
  ...
  def show
    url = ENV["API_URL"]+"players/#{params["id"]}.json"
    connection = Faraday.get url
    @player = JSON.parse(connection.body).first

    # SEO
    @page_title       = t('.title', player: @player['name'], season: ENV["CURRENT_SEASON"])
    @page_description = t('.description', player: @player['name'], season: ENV["CURRENT_SEASON"])
    @page_keywords    = t('.keywords', player: @player['name'])
    # SEO

    #fields = ['week_1', 'week_2', 'week_3', 'week_4', 'week_5']
    fields = (1...ENV["CURRENT_ROUND"].to_i).map{ |i| 'week_'+i.to_s}

    prices = @player['price']
    prices = prices.except!("0")
    prices = prices.except!(ENV["CURRENT_SEASON"])
    ...
    @stats_data = { 
      labels:(1...ENV["CURRENT_ROUND"].to_i).map{|key| [t("round"), key.to_s].join(" ")}, 
      datasets: [
        { label: "SuperManager", 
  ...
  def brokerbasket
    ...
    connection = Faraday.get ENV["API_URL"]+"players?season=#{ENV["CURRENT_SEASON"]}&position=base"
    @players_bases = JSON.parse(connection.body)

    connection = Faraday.get ENV["API_URL"]+"players?season=#{ENV["CURRENT_SEASON"]}&position=alero"
    @players_aleros = JSON.parse(connection.body)

    connection = Faraday.get ENV["API_URL"]+"players?season=#{ENV["CURRENT_SEASON"]}&position=pivot"
  ...
  def historico
    ...
    unless @position == "all"
      connection = Faraday.get ENV["API_URL"]+"players?season=#{ENV["CURRENT_SEASON"]}&position=#{@position}"
      @players = JSON.parse(connection.body)
    end
  ...
  def temporada
    ...
    #params
    @num_rounds = ENV["CURRENT_ROUND"].to_i-1
    @position = params[:position] ? params[:position] : "all"
    @field = params[:field] || "v"
  ...
  def search
    url = ENV["API_URL"]+"players?name=#{params[:q]}"
    connection = Faraday.get url
    @players = JSON.parse(connection.body)
